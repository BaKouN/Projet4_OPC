...
<div class="container-fluid brand-bg py-5">
	<div class="container">
		...
		<button type='button' class='postBtn btn btn-primary'>Mettre à jour ce billet</button>
	</div>
</div>
...
<script>
	...
	$('.postBtn').click(() => // UPDATE TICKET ON CLICK
	{	
		let title = $('#title').val();
		let content = $('#trumbowyg-demo').trumbowyg('html');
		if (!title.length || !content.length) return;
		$.ajax({
			url: '<?= $GLOBALS['websitePath'] ?>/api/post/<?= $postID ?>/update',
			method: "POST",
			data: {
				title: title,
				content: content
			...
			if (data == 1) 
				var modal = container.addMessage("Billet mis à jour avec succès !");
			else
				var modal = container.addMessage("Erreur ! Vérifiez la bonne mise à jour du billet !");
		});
	});
</script>
